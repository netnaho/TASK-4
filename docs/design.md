...
- Downloads trigger explicit event tracking and dynamic preview watermarking via frontend overlays. Server-side SHA-256 signatures are persistently tracked for content integrity verifications.

### 6.3 Field Evidence Check-Ins
- Purpose built for site logging (e.g., coordinates, inspector notes, multimedia evidence arrays).
- Employs a fully versioned update mechanism. User modifications initiate new revisions with previous delta fields maintained, rendering inspection overlays immediately visible.

### 6.4 Dual Approval Mechanics (Critical Actions)
- System actions tagged as critical risk (Order Cancellations, Document Destruction, Override Approvals) mandate authorization by secondary users holding adequate systemic privileges.
...
- **Environment Parity:** The backend explicitly requires `mvnw` (Maven Wrapper) files to allow normalized command-line testing absent Docker or globally installed binaries.
